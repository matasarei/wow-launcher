...
| `SPATIAL_AUDIO=1` | Apple spatial audio for headphones (Audio pane): `wow-launch` exports `WOWSILICON_SPATIAL_AUDIO_MODE=fixed` (else `off`); takes effect at the next game start |
| `NORMALIZE_AUDIO=1` | volume normalizer — quiet up, loud down (Audio pane): exports `WOWSILICON_NORMALIZE_AUDIO=1` (else `0`); next game start |
