...
# audio (runtime r15 winecoreaudio contract): off/0 exported explicitly by default,
# device following left to the driver's default, control files pinned inside the bundle
assert_contains "$(cat "$WINELOG")" "SPATIAL=off" "spatial audio off by default"
assert_contains "$(cat "$WINELOG")" "NORM=0" "normalize audio off by default"
...
printf 'SPATIAL_AUDIO=1\nNORMALIZE_AUDIO=1\n' >> "$RES/launcher.conf"
: > "$WINELOG"; WOWSILICON_SPATIAL_AUDIO_MODE=garbage "$BIN/wow-launch"; sleep 0.3
assert_contains "$(cat "$WINELOG")" "SPATIAL=fixed" "SPATIAL_AUDIO=1 turns the spatial mixer on (and overrides a stale shell value)"
assert_contains "$(cat "$WINELOG")" "NORM=1" "NORMALIZE_AUDIO=1 turns the normalizer on"
sed -i '' 's/^SPATIAL_AUDIO=.*/SPATIAL_AUDIO=0/; s/^NORMALIZE_AUDIO=.*/NORMALIZE_AUDIO=yes/' "$RES/launcher.conf"
: > "$WINELOG"; "$BIN/wow-launch"; sleep 0.3
assert_contains "$(cat "$WINELOG")" "SPATIAL=off" "SPATIAL_AUDIO=0 is off"
assert_contains "$(cat "$WINELOG")" "NORM=0" "anything but NORMALIZE_AUDIO=1 is off"
