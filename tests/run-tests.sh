...
assert_contains "$OUT" "Ancient 2.0 is too old to install from" "1.0/2.0 (old bundle id) is refused"
for ver in 2.0 1.9 2 garbage ""; do
  ...
  assert_contains "$OUT" "too old to install from" "version '$ver' is refused"
done
...
G="$RES/games/main"
assert_contains "$OUT" "installing from Old Launcher 2.10" "the previous app is accepted"
assert_contains "$OUT" "game installed (3.3.5a) from Old Launcher 2.10" "and its game installed"
