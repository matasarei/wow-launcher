...
- When touching `wow-client-profile` or the capability gates: run it against a
  real client and check `BUILD`/`CONFIDENCE` (a stock 3.3.5a must report
  `BUILD=12340 CONFIDENCE=exact`), and confirm the Game pane still offers all
  four patch levels for it. The hermetic suite covers the shapes; only a real
  client proves the version-resource read.
...
| `.github/workflows/` | CI: tests, Swift compile, shellcheck, strings parity, tag/version match; monthly upstream-pin check |
