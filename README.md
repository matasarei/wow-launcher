...
- **Game** — install a client (pick any client folder — what it is gets detected, it's copied in, and it gets the Apple Silicon patches that apply to it, then it's verified), or **update from a previous version**: *Install from Previous App…* takes the game — already patched, with your settings, addons and language packs — from the older WoW Launcher app still in your Applications (2.1 or later, any name) and only copies and verifies it; the old app is left untouched, so delete it once the new one plays. Also: choose how much of the patch stack to apply (**Patches**: the menu lists only the levels this client can take), **Verify** integrity (client-aware checks: files, patches, settings — 43 of them for 3.3.5a) with one-click **Fix Issues** repair, or a reinstall suggestion if game data is damaged beyond repair. Below: the **language packs** (3.3.5a/2.4.3) — import a pack from a client in another language and switch the game language from a dropdown (the pack's data and matching executable are swapped, the cache cleared); and the **server list** editor (realmlist.wtf) — radio-select the active server, add or remove entries.
...
- **Stuck at "Connecting" on a local-network server** (a `192.168.x.x` /
  ...
  permission belongs to the launcher — on macOS 27 the game loses the
  connection when the launcher really quits — so closing it while the game
  runs (Cmd+Q, or **Close the launcher when the game starts** on the Play
  screen) only takes it off the screen; it quits by itself when you exit the
  game. To get it back mid-game, open the app again. Every rebuilt, renamed or
  moved copy of the app is a new app to macOS and has to be allowed again. Run the app from `Applications` (not
